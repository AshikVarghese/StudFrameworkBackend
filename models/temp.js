...
function club(params,callback) {
    connection.query("SELECT * FROM ec_club_activity WHERE roll_no = ?",[params.RollNumber],
    (err, results, fields) => {
        if(err){
            console.log(err)
        }
...
function cultural(params, callback) {
    connection.query("SELECT * FROM ec_culturals WHERE roll_no = ?",[params.RollNumber],
    (err, results, fields) => {
        if(err){
            console.log(err)
        }
...
function outreach(params,callback) {
    connection.query("SELECT * FROM ec_outreach WHERE roll_no = ?",[params.RollNumber],
    (err, results, fields) => {
        if(err){
            console.log(err)
        }
...
function sports(params,callback) {
    connection.query("SELECT * FROM ec_sports WHERE roll_no = ?",[params.RollNumber],
    (err, results, fields) => {
        if(err){
            console.log(err)
        }
