...
function PdMini_Stud_display(callback) {
  connection.query(
    "SELECT * FROM `pd_mini_project` inner join student_details on pd_mini_project.roll_no = student_details.roll_no WHERE(roll_no=?)",
...
function PdMini_Stud_insert(callback) {
  connection.query(
    ...
    [params.StudentDetails, params.Title, params.Objective, params.Outcome],
    (err, results, fields) => {
      if (err) {
        console.log(err);
